# Edge15 AI Oracle v1 — 6 Minute Lock + 100% Defense + Normalized Selector
...
- Ranks BTC, ETH, SOL, BNB, and XRP using a normalized multi-coin selector score instead of raw dollar distance alone.
...
## New in normalized selector build

This build keeps the v1 6-minute/Defense model intact but changes how markets are ranked.

The app now uses a **Normalized Selector Score** so BTC does not win just because its dollar distance from target is larger. The selector compares each coin by:

- percent distance from target
- distance relative to that coin's recent 1-minute movement
- momentum agreement
- local range agreement
- settlement/flip risk

This should give ETH, SOL, BNB, and XRP a fairer chance to become the best ranked opportunity when their setup is cleaner than BTC. The official lock still uses the same v1-style Edge Score thresholds and the same 7:00 / 6:00 / 4:00 windows.
