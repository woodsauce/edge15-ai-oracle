...
function makeForecast(input) {
  ...
  if (!['OVER', 'UNDER'].includes(direction)) {
    return { symbol, pick: 'WAIT', probability: 18, projectedWindow: 'No lock', risk: 'Very High', action: 'Wait', note: 'No stable direction yet.' };
  }
  ...
  return { symbol, pick: direction, probability, projectedWindow, risk, action, note };
}
...
function resolveScoutSnapshots() {
  ...
  for (const snap of state.scout.snapshots || []) {
    ...
    if (official.result === 'S') {
      snap.status = 'faded';
      snap.result = 'F';
    } else if (official.symbol === snap.symbol && official.pick === snap.pick) {
...
function renderScoutList() {
  ...
  els.scoutList.innerHTML = snaps.map(s => `
    <div class="scout-item ${s.status}">
      <div><strong>${s.bucket} • ${s.symbol} ${s.pick}</strong><span>${s.probability}% • score ${s.score} • projected ${s.projectedWindow}</span></div>
      <strong>${s.result || s.status.toUpperCase()}</strong>
    </div>
  `).join('');
}
...
function renderScoutStats() {
  const snaps = state.scout.snapshots || [];
  const wins = snaps.filter(s => s.status === 'win').length;
  const losses = snaps.filter(s => s.status === 'loss').length;
  const fades = snaps.filter(s => s.status === 'faded').length;
  const total = wins + losses;
...
function exportAllData() {
  ...
  downloadJson(payload, `edge15-ai-oracle-v2-export-${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
}
