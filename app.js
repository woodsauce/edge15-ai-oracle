...
const BOT_RECORDS_KEY = 'edge15-ai-oracle-bot-records-v1';

const DEFAULT_BOT_SETTINGS = {
  botMode: 'demo',
  demoOrderMode: 'kalshi-demo',
  strategyMode: 'champion-mirror',
  maxTradeSize: 1,
  maxTradesPerDay: 0,
  maxDailyLoss: 5,
  maxOpenPositions: 1,
  ...
  sixOnly: false,
  allow4: true,
  useExtraPredictionFilters: false,
  minScore: 70,
  minSelectorScore: 72,
  allowedRisk: 'Medium',
  armedStartMin: 4.5,
...
const DEFAULT_BOT_RECORDS = {
  championMirror: { label: 'Champion Mirror 1:1', wins: 0, losses: 0, trades: 0, blocks: 0, history: [] },
  armedWindow: { label: 'Armed Window 4:30–6:30', wins: 0, losses: 0, trades: 0, blocks: 0, history: [] }
};
...
const state = {
  ...
  botRecords: normalizeBotRecords(load(BOT_RECORDS_KEY, DEFAULT_BOT_RECORDS)),
  loading: false,
  lastRoundKey: '',
  abort: null
};
...
const els = {
  ...
  botStrategyMode: document.getElementById('botStrategyMode'),
  ...
  botArmedStartMin: document.getElementById('botArmedStartMin'),
  botArmedEndMin: document.getElementById('botArmedEndMin'),
  ...
  botUseExtraPredictionFilters: document.getElementById('botUseExtraPredictionFilters'),
  ...
  botRecordSummary: document.getElementById('botRecordSummary'),
  botSaveSettings: document.getElementById('botSaveSettings'),
  botTestKeys: document.getElementById('botTestKeys'),
  botKeyTestResult: document.getElementById('botKeyTestResult'),
  ...
  botResetRecords: document.getElementById('botResetRecords'),
...
function init() {
  ...
  els.botTestKeys?.addEventListener('click', testKalshiKeys);
  ...
  els.botResetRecords?.addEventListener('click', () => { if (confirm('Reset Champion Mirror and Armed Window bot records?')) { state.botRecords = normalizeBotRecords(DEFAULT_BOT_RECORDS); save(BOT_RECORDS_KEY, state.botRecords); renderBotPanel(); } });
...
async function scan() {
  ...
  try {
    ...
    resolvePendingLocks();
    resolvePendingBotTrades();
    maybeLockBest();
    maybeArmedWindowTrade();
    render();
...
function maybeLockBest() {
  ...
  if (!defenseBlocked && best.edgeScore >= threshold && ['OVER', 'UNDER'].includes(best.direction)) {
    const lock = {
      ...
      lockedAt: Date.now(),
      timeMinAtLock: timeMin,
      strategySource: 'official-lock',
...
function hydrateBotSettings() {
  ...
  if (els.botStrategyMode) els.botStrategyMode.value = s.strategyMode ?? 'champion-mirror';
  ...
  if (els.botArmedStartMin) els.botArmedStartMin.value = s.armedStartMin ?? 4.5;
  if (els.botArmedEndMin) els.botArmedEndMin.value = s.armedEndMin ?? 6.5;
  ...
  if (els.botUseExtraPredictionFilters) els.botUseExtraPredictionFilters.checked = Boolean(s.useExtraPredictionFilters);
...
function readBotSettingsFromUi() {
  state.botSettings = {
    ...
    strategyMode: els.botStrategyMode?.value || 'champion-mirror',
    ...
    useExtraPredictionFilters: Boolean(els.botUseExtraPredictionFilters?.checked),
    ...
    armedStartMin: clamp(number(els.botArmedStartMin?.value) || 4.5, 0, 15),
    armedEndMin: clamp(number(els.botArmedEndMin?.value) || 6.5, 0, 15),
...
function strategyKey(strategyMode = state.botSettings.strategyMode) {
  return strategyMode === 'armed-window' ? 'armedWindow' : 'championMirror';
}

function strategyLabel(strategyMode = state.botSettings.strategyMode) {
  return strategyMode === 'armed-window' ? 'Armed Window 4:30–6:30' : 'Champion Mirror 1:1';
}

function normalizeBotRecords(records) {
  const base = JSON.parse(JSON.stringify(DEFAULT_BOT_RECORDS));
  const input = records || {};
  for (const key of Object.keys(base)) {
    base[key] = { ...base[key], ...(input[key] || {}) };
    base[key].history = Array.isArray(base[key].history) ? base[key].history : [];
    base[key].wins = Number(base[key].wins) || 0;
    base[key].losses = Number(base[key].losses) || 0;
    base[key].trades = Number(base[key].trades) || 0;
    base[key].blocks = Number(base[key].blocks) || 0;
  }
  return base;
}

function botRoundAlreadyTraded(roundKey, strategyMode) {
  return state.botLog.some(t => t.roundKey === roundKey && t.strategyMode === strategyMode && ['submitted', 'simulated', 'filled', 'won', 'lost'].includes(t.status));
}

function botLockAlreadyLogged(lockId) {
  return state.botLog.some(t => t.lockId === lockId);
}

function applyOptionalPredictionFilters(lock) {
  const b = state.botSettings;
  if (!b.useExtraPredictionFilters) return '';
  if (b.sixOnly && lock.window !== '6:00') return 'Extra filter: 6:00-only mode blocked this non-6:00 lock.';
  if (!b.allow4 && lock.window === '4:00') return 'Extra filter: 4:00 backup locks are disabled.';
  if (lock.score < b.minScore) return `Extra filter: edge score ${lock.score} is below minimum ${b.minScore}.`;
  if ((lock.selectorScore ?? 0) < b.minSelectorScore) return `Extra filter: selector score ${lock.selectorScore ?? 0} is below minimum ${b.minSelectorScore}.`;
  if (!riskAllowed(lock.risk, b.allowedRisk)) return `Extra filter: risk ${lock.risk} is above allowed ${b.allowedRisk}.`;
  return '';
}

function botBlockReason(lock, market) {
  const b = state.botSettings;
  const mode = b.strategyMode || 'champion-mirror';
  const timeMin = Number.isFinite(number(lock.timeMinAtLock)) ? number(lock.timeMinAtLock) : ((number(lock.closeMs) - Date.now()) / 60000);
  const start = Math.min(number(b.armedStartMin) || 4.5, number(b.armedEndMin) || 6.5);
  const end = Math.max(number(b.armedStartMin) || 4.5, number(b.armedEndMin) || 6.5);
  ...
  if (botRoundAlreadyTraded(lock.roundKey, mode)) return `${strategyLabel(mode)} already acted on this round.`;

  if (mode === 'armed-window') {
    if (!(timeMin >= start && timeMin <= end)) return `Armed Window mode only trades between ${start.toFixed(1)} and ${end.toFixed(1)} minutes remaining.`;
  }

  const predictionFilterBlock = applyOptionalPredictionFilters(lock);
  if (predictionFilterBlock) return predictionFilterBlock;
...
function recordBotBlock(strategyMode, lock, reason) {
  const key = strategyKey(strategyMode);
  const record = state.botRecords[key];
  if (!record) return;
  record.blocks += 1;
  record.history.unshift({ result: 'B', roundKey: lock.roundKey, symbol: lock.symbol, pick: lock.pick, window: lock.window, score: lock.score, reason, ts: Date.now() });
  record.history = record.history.slice(0, 250);
  save(BOT_RECORDS_KEY, state.botRecords);
}

function recordBotOutcome(logEntry, result) {
  if (!logEntry || logEntry.recordedOutcome) return;
  const key = strategyKey(logEntry.strategyMode);
  const record = state.botRecords[key];
  if (!record) return;
  if (result === 'W') record.wins += 1;
  else if (result === 'L') record.losses += 1;
  record.trades += 1;
  record.history.unshift({ result, roundKey: logEntry.roundKey, symbol: logEntry.symbol, pick: logEntry.pick, window: logEntry.window, score: logEntry.score, selectorScore: logEntry.selectorScore, ts: Date.now() });
  record.history = record.history.slice(0, 250);
  logEntry.recordedOutcome = true;
  save(BOT_RECORDS_KEY, state.botRecords);
}

async function resolvePendingBotTrades() {
  const open = state.botLog.filter(t => ['submitted', 'simulated', 'filled'].includes(t.status) && !t.outcome && t.closeMs && Date.now() > Number(t.closeMs) + 35000);
  for (const trade of open.slice(0, 5)) {
    try {
      const data = await fetchJson(`/api/kalshi-market?ticker=${encodeURIComponent(trade.marketTicker)}`);
      const m = data?.market || {};
      let outcome = null;
      if (m.result === 'yes') outcome = 'OVER';
      if (m.result === 'no') outcome = 'UNDER';
      if (!outcome && Number.isFinite(number(m.expirationValue)) && Number.isFinite(number(trade.target))) {
        outcome = number(m.expirationValue) >= number(trade.target) ? 'OVER' : 'UNDER';
      }
      if (outcome) {
        const result = outcome === trade.pick ? 'W' : 'L';
        trade.outcome = result;
        trade.status = result === 'W' ? 'won' : 'lost';
        trade.actual = outcome;
        trade.finalValue = m.expirationValue ?? m.expirationValueRaw;
        trade.resolvedAt = new Date().toISOString();
        recordBotOutcome(trade, result);
      }
    } catch (err) {
      console.warn('Bot trade resolve failed', err);
    }
  }
  save(BOT_LOG_KEY, state.botLog.slice(0, 250));
}

function maybeArmedWindowTrade() {
  const b = state.botSettings;
  if ((b.strategyMode || 'champion-mirror') !== 'armed-window') return;
  const best = state.markets[0];
  if (!best || !['OVER', 'UNDER'].includes(best.direction)) return;
  const timeMin = best.timeRemaining / 60000;
  const start = Math.min(number(b.armedStartMin) || 4.5, number(b.armedEndMin) || 6.5);
  const end = Math.max(number(b.armedStartMin) || 4.5, number(b.armedEndMin) || 6.5);
  if (!(timeMin >= start && timeMin <= end)) return;
  const roundKey = getRoundKey(best.closeMs);
  if (botRoundAlreadyTraded(roundKey, 'armed-window')) return;
  const lock = {
    id: `armed-window-${roundKey}`,
    roundKey,
    marketTicker: best.kalshi?.ticker,
    symbol: best.symbol,
    pick: best.direction,
    ...
    selectorScore: best.selectorScore,
    risk: best.risk,
    window: `Armed ${timeMin.toFixed(1)}m`,
    target: best.target,
    priceAtLock: best.price,
    closeMs: best.closeMs,
    lockedAt: Date.now(),
    timeMinAtLock: timeMin,
    reason: `Armed Window mode acted inside ${start.toFixed(1)}–${end.toFixed(1)} minutes on the current champion selector leader.`,
    setupKey: setupKey(best.symbol, best.direction, best.absDistance, timeMin),
    strategySource: 'armed-window',
    status: 'bot-only'
  };
  maybeAutoTrade(lock, best);
}
...
async function maybeAutoTrade(lock, market) {
  ...
  if (botLockAlreadyLogged(lock.id)) return;
  const strategyMode = state.botSettings.strategyMode || 'champion-mirror';
  const block = botBlockReason(lock, market);
  if (block) {
    addBotLog({ lockId: lock.id, roundKey: lock.roundKey, symbol: lock.symbol, pick: lock.pick, window: lock.window, score: lock.score, selectorScore: lock.selectorScore, risk: lock.risk, strategyMode, strategyLabel: strategyLabel(strategyMode), status: 'blocked', reason: block });
    recordBotBlock(strategyMode, lock, block);
    renderBotPanel();
    return;
  }
  ...
  const base = {
    ...
    target: lock.target,
    closeMs: lock.closeMs,
    timeMinAtLock: lock.timeMinAtLock,
    source: lock.strategySource || 'official-lock',
    dayKey: botDayKey(),
  ...
  try {
    ...
    if (!response.ok || data.error) {
      addBotLog({ ...base, status: 'error', reason: data.error || `Order endpoint failed ${response.status}`, detail: data.detail || data.message || '', code: data.code || '', diagnostics: data.diagnostics || data.authDiagnostics || {}, attempts: data.attempts || [] });
    } else {
...
function renderBotPanel() {
  ...
  els.botStatusPill.textContent = `BOT: ${String(b.botMode || 'off').toUpperCase()} • ${strategyLabel(b.strategyMode)}`;
  ...
  if (els.botRecordSummary) els.botRecordSummary.innerHTML = renderBotRecordCards();
  els.botTradeLog.innerHTML = state.botLog.slice(0, 18).map(t => `
    <div class="bot-log-item ${escapeHtml(t.status || '')}">
      <div><strong>${escapeHtml(t.status || 'log')}</strong><span>${new Date(t.ts || Date.now()).toLocaleTimeString()} • ${escapeHtml(t.strategyLabel || '')} • ${escapeHtml(t.symbol || '')} ${escapeHtml(t.pick || '')} ${escapeHtml(t.window || '')}</span></div>
      <p>${escapeHtml(t.reason || '')}${t.outcome ? ` • outcome ${escapeHtml(t.outcome)}` : ''}</p>
    </div>
  `).join('') || '<p class="small-note">No bot trades yet.</p>';
}

function renderBotRecordCards() {
  return Object.entries(state.botRecords).map(([key, rec]) => {
    const total = rec.wins + rec.losses;
    const acc = total ? `${Math.round((rec.wins / total) * 1000) / 10}%` : '--';
    const last = (rec.history || []).slice(0, 10).map(h => `<span title="${escapeHtml(h.symbol || '')} ${escapeHtml(h.pick || '')} ${escapeHtml(h.reason || '')}" class="dot ${h.result === 'W' ? 'win' : h.result === 'L' ? 'loss' : 'skip'}">${escapeHtml(h.result || 'B')}</span>`).join('');
    return `<div class="bot-record-card"><strong>${escapeHtml(rec.label || key)}</strong><div>${rec.wins}W / ${rec.losses}L • ${acc}</div><small>${rec.trades || 0} settled trades • ${rec.blocks || 0} blocks</small><div class="last10">${last}</div></div>`;
  }).join('');
}


async function testKalshiKeys() {
  readBotSettingsFromUi();
  save(BOT_SETTINGS_KEY, state.botSettings);
  const mode = state.botSettings.botMode === 'live' ? 'live' : 'demo';
  if (els.botKeyTestResult) {
    els.botKeyTestResult.className = 'key-test-box running';
    els.botKeyTestResult.textContent = `Testing ${mode.toUpperCase()} Kalshi key setup...`;
  }
  try {
    const response = await fetch(`/api/kalshi-key-test?mode=${encodeURIComponent(mode)}`, { method: 'GET' });
    const data = await response.json().catch(() => ({}));
    const checks = data.checks || {};
    const attempts = Array.isArray(data.attempts) ? data.attempts : [];
    const lines = [
      `Mode: ${String(data.mode || mode).toUpperCase()}`,
      `API Key ID found: ${checks.keyIdFound ? 'YES' : 'NO'}${data.diagnostics?.keyIdSuffix ? ` • ends ${data.diagnostics.keyIdSuffix}` : ''}`,
      `Private key found: ${checks.privateKeyFound ? 'YES' : 'NO'}`,
      `Private key parsed: ${checks.privateKeyParsed ? 'YES' : 'NO'}`,
      `Signature created: ${checks.signatureCreated ? 'YES' : 'NO'}`,
      `Kalshi auth request: ${checks.authRequestSucceeded ? 'PASS' : 'FAIL'}`
    ];
    if (data.diagnostics?.selectedSource) lines.push(`Key source used: ${data.diagnostics.selectedSource}`);
    if (data.diagnostics?.selectedHeader) lines.push(`Detected header: ${data.diagnostics.selectedHeader}`);
    if (attempts.length) {
      const last = attempts[attempts.length - 1];
      if (last?.error) lines.push(`Parser detail: ${last.error}`);
    }
    if (Array.isArray(data.notes) && data.notes.length) lines.push(`Notes: ${data.notes.join(' | ')}`);
    if (data.error) lines.push(`Error: ${data.error}`);
    if (els.botKeyTestResult) {
      els.botKeyTestResult.className = `key-test-box ${data.ok ? 'pass' : 'fail'}`;
      els.botKeyTestResult.textContent = lines.join('\n');
    }
    addBotLog({ status: data.ok ? 'key_test_pass' : 'key_test_fail', reason: data.ok ? 'Kalshi key test passed.' : 'Kalshi key test failed.', detail: lines.join(' | ') });
  } catch (err) {
    const msg = err.message || 'Kalshi key test failed.';
    if (els.botKeyTestResult) {
      els.botKeyTestResult.className = 'key-test-box fail';
      els.botKeyTestResult.textContent = msg;
    }
    addBotLog({ status: 'key_test_fail', reason: msg });
  }
  renderBotPanel();
}
