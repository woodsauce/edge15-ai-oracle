...
function analyzeFeatures(symbol, price, target, candles, market, timeRemaining) {
  ...
  const reasons = buildReasons({ symbol, direction, targetSide, momentumSide, trendSide, wickSide, oddsSide, absDistance, meta, momentum, bodyRatio, settlementPenalty, edgeScore, target, price, defenseBlock });
  const council = [
  ...
  return { edgeScore, direction, risk, status, reasons, council, momentum, momentumSide, trendSide, wickSide, rangePos, bodyRatio, upperWick, lowerWick, targetDistance, absDistance, distanceBand, settlementPenalty, defenseBlock }; 
}
...
function buildReasons(input) {
  const { direction, targetSide, momentumSide, trendSide, wickSide, oddsSide, absDistance, meta, momentum, bodyRatio, settlementPenalty, edgeScore, target, price, defenseBlock } = input;
  const reasons = [];
